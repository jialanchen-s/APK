...
export class EstimateTaskService {
  ...
  async createTask(
    ...
    this.logger.log(
      `createTask: userId=${userId}, fileName=${data.file_name}, lineType=${data.line_type}, rows=${data.rows.length}`,
    );
    ...
    this.processTask(taskId, data.rows, data.line_type, estimateMode, targetProjects).catch((err) => {
  ...
  private async processTask(
    ...
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.all(
        batch.map((row) => this.processRow(taskId, row, taskLineType, publishedModels, this.aiParamsCache, estimateMode, targetProjects)),
      );
      for (let j = 0; j < results.length; j++) {
        rowResults[i + j] = results[j];
        if (results[j].status === 'success') successCount++;
        else if (results[j].status === 'jia_gong') jiaGongCount++;
        else pendingCount++;
      }
    }
  ...
  private async processRow(
    ...
    if (matchedModel && matchedModel.formulaLogic) {
      ...
      await this.db.insert(pendingItems).values({
        taskId,
  ...
  async getContractProjects(): Promise<ContractProjectsResponse> {
    const rows = await this.db
      .select({ project: contracts.project })
      .from(contracts)
      .where(eq(contracts.status, 'approved'));
  ...
  private async findContractMatch(
    ...
    const matchConditions = [
      eq(contracts.deviceMaterialName, deviceName),
      eq(contracts.lineType, lineType),
      eq(contracts.distinction, modifyLevel),
      eq(contracts.status, 'approved'),
    ];
    if (estimateMode === 'project_match' && targetProjects.length > 0) {
      matchConditions.push(inArray(contracts.project, targetProjects));
    }

    const orderBy = estimateMode === 'budget'
      ? desc(contracts.unitPrice)
      : desc(contracts.archiveTime);
    const limit = (estimateMode === 'budget' || estimateMode === 'project_match') ? 100 : 100;

    const matched = await this.db
      .select({
        id: contracts.id,
        unitPrice: contracts.unitPrice,
        project: contracts.project,
        lineType: contracts.lineType,
        archiveTime: contracts.archiveTime,
        selectedBrand: contracts.selectedBrand,
      })
      .from(contracts)
    ...
    const fuzzyConditions = [
      like(contracts.deviceMaterialName, `%${deviceName}%`),
      eq(contracts.lineType, lineType),
      eq(contracts.distinction, modifyLevel),
      eq(contracts.status, 'approved'),
    ];
    if (estimateMode === 'project_match' && targetProjects.length > 0) {
      fuzzyConditions.push(inArray(contracts.project, targetProjects));
    }
  ...
  private async findBestCrossLineMatch(
    ...
    if (excludeLineType) {
      conditions.push(ne(contracts.lineType, excludeLineType));
    }
    if (estimateMode === 'project_match' && targetProjects.length > 0) {
      conditions.push(inArray(contracts.project, targetProjects));
    }
    ...
    if (excludeLineType) {
      fuzzyConditions.push(ne(contracts.lineType, excludeLineType));
    }
  ...
  private aiCallQueue: Promise<Record<string, string | number>> = Promise.resolve({});
  private aiInFlight = new Map<string, Promise<Record<string, string | number>>>();

  /** AI参数提取（带超时保护、内存缓存、限流重试、并发串行化） */
  private async extractAiParams(
    ...
    const promise = this.aiCallQueue.then(() => this.doExtractAiParams(text, timeoutMs));
    this.aiInFlight.set(text, promise);
    this.aiCallQueue = promise.catch(() => ({}));
    try {
  ...
  private async doExtractAiParams(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        const aiPromise = this.capabilityService
          .load('welding_equipment_param_extract_1')
          .call('textToJson', { equipment_text: text });
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('AI提取超时')), timeoutMs),
        );
        const output = await Promise.race([aiPromise, timeoutPromise]) as WeldingEquipmentParamExtractOneOutput;
  ...
  private async runAnomalyDetection(
    ...
    try {
      const output = await this.capabilityService
        .load('welding_cost_anomaly_detection_1')
        .call('textToJson', { comparison_text: comparisonText }) as WeldingCostAnomalyDetectionOneOutput;
