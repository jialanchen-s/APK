...
- 原 9 个 aPaaS 插件实例已全部迁移至 AI Gateway 本地实现（`gateway.chat()` / `gateway.textToJson()`），不再依赖平台插件能力。迁移后输出类型保留在 `shared/api.interface.ts`（`WeldingEquipmentParamExtractOneOutput`、`WeldingCostAnomalyDetectionOneOutput`）。
- textToJson 系列任务的输出字段全部必填，prompt 已要求缺失字段填 0/空字符串，禁止改动为返回 null。
- textToJson 系列任务的 prompt 应包含字段的详细说明，确保 LLM 输出正确。
