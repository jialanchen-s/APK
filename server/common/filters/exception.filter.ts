...
export class GlobalExceptionFilter implements ExceptionFilter {
    ...
    if (exception instanceof BusinessException) {
      ...
      // 未知异常：服务端记录完整堆栈与 cause 便于排障，响应不暴露内部细节
      const detail = exception instanceof Error
        ? `${exception.stack ?? exception.name}: ${exception.message}${
            (exception as { cause?: unknown }).cause !== undefined
              ? `; cause=${JSON.stringify((exception as { cause?: unknown }).cause)}`
              : ''
          }`
        : JSON.stringify(exception);
      new Logger('GlobalExceptionFilter').error(`Unhandled exception: ${detail}`);
