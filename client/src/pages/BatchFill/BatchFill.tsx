  ...
  const handleSubmit = async () => {
    ...
    try {
      const submitItems = items.map((item: PendingItem) => {
        ...
        const { _manual_price: manualRaw, ...paramEdits } = edits;
        ...
        const manualNum = Number(manualRaw);
        const manualPrice =
          manualRaw !== undefined && manualRaw !== '' && Number.isFinite(manualNum) && manualNum > 0
            ? manualNum
            : undefined;
        return { id: item.id, params: values, manual_price: manualPrice };
      });
